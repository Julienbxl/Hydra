#!/usr/bin/env python3
"""testpass.py — 10 tests mode PASSPHRASE
Generates BIP39 phrases + base passwords, derives targets for Direct and Rule-based paths,
and verifies Hydra --passphrase finds all of them.
"""
import hashlib,hmac,os,random,struct,subprocess,sys,tempfile,string
...
def gen_passphrase(min_len=4, max_len=10):
...
def run_test(hydra_bin: str, targets: list) -> bool:
    print(f"=== BRAINWALLET/PASSPHRASE AUTOMATED TEST SUITE ===")
    print(f"--- Running {len(targets)} tests ---")
    
    dict_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'resources', 'dictionary.txt')
    rule_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'resources', 'rule.txt')
    backup_dict = None
    backup_rule = None
    
    if os.path.exists(dict_path):
        fd, backup_dict = tempfile.mkstemp(suffix='.txt', dir='.', prefix='dict_bak_')
        os.close(fd); os.unlink(backup_dict)
        os.replace(dict_path, backup_dict)
    ...
    try:
        ...
        if os.path.exists(dict_path): os.unlink(dict_path)
        if os.path.exists(rule_path): os.unlink(rule_path)
        if backup_dict and os.path.exists(backup_dict): os.replace(backup_dict, dict_path)
...
def main():
    ...
    for i in range(10):
        base = gen_passphrase()
        phrase = ' '.join(gen_phrase(24 if i >= 7 else 12))
        
        # Determine rule mapping for i (0=direct, 1=upper, 2=cap, 3=append!, 4-9=random)
        r = i if i < 4 else random.randint(0, 3)
        ...
        if r == 0:
            ...
            expected = base.upper()
            t_type = "Rule: Uppercase"
        elif r == 2:
            expected = base.capitalize()
            t_type = "Rule: Capitalize"
        elif r == 3:
            expected = base + "!"
            t_type = "Rule: Append !"
            
        coin = 60 if i % 2 == 1 else 0
        at = 'eth' if coin == 60 else 'btc'
        addr = leaf_addr(phrase, expected, coin, at)
    ...
    success = run_test(hydra, targets)
    if success:
