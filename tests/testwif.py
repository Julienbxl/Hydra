#!/usr/bin/env python3
"""testwif.py — 10 tests mode WIF (monolithique, stdlib)
Usage : python3 testwif.py [./Hydra]
"""
...
def pub_uncompressed(k): _,px,py=pub(k); return b'\x04'+px.to_bytes(32,'big')+py.to_bytes(32,'big')
...
def btc_legacy_uncompressed(k): h=h160(pub_uncompressed(k));pl=b'\x00'+h;return b58e(pl+sha2d(pl)[:4])
def btc_segwit(k):
...
def wif_enc(kb): pl=b'\x80'+kb+b'\x01';return b58e(pl+sha2d(pl)[:4])
def wif_uncompressed_enc(kb): pl=b'\x80'+kb;return b58e(pl+sha2d(pl)[:4])

HYDRA=resolve_hydra()
NUM_TESTS=10; NUM_UNKN=5
ATYPES=[
    'btc_legacy',
    'btc_segwit',
    'btc_legacy_uncompressed',
    'pubkey_compressed_wif_compressed',
    'pubkey_uncompressed_wif_compressed',
    'pubkey_compressed_wif_uncompressed',
    'pubkey_uncompressed_wif_uncompressed',
]

def gen(i):
    k=random.randint(1,N-1); kb=k.to_bytes(32,'big')
    at=ATYPES[i%len(ATYPES)]
    p,px,py=pub(k)
    if at=='btc_legacy_uncompressed':
        wif=wif_uncompressed_enc(kb); addr=btc_legacy_uncompressed(k)
    elif at=='pubkey_compressed_wif_compressed':
        wif=wif_enc(kb); addr=p.hex()
    elif at=='pubkey_uncompressed_wif_compressed':
        wif=wif_enc(kb); addr=pub_uncompressed(k).hex()
    elif at=='pubkey_compressed_wif_uncompressed':
        wif=wif_uncompressed_enc(kb); addr=p.hex()
    elif at=='pubkey_uncompressed_wif_uncompressed':
        wif=wif_uncompressed_enc(kb); addr=pub_uncompressed(k).hex()
    else:
        wif=wif_enc(kb); addr=btc_segwit(k) if at=='btc_segwit' else btc_legacy(k)
    s=random.randint(1,len(wif)-NUM_UNKN)
    return dict(i=i,kb=kb,wif=wif,addr=addr,at=at,mask=wif[:s]+'#'*NUM_UNKN+wif[s+NUM_UNKN:],s=s)

def run(t):
    print(f"\n{'='*62}\nTest #{t['i']+1:02d} | {t['at']}\n  WIF    : {t['wif']}\n  Masque : {t['mask']} (pos {t['s']}..{t['s']+NUM_UNKN-1})\n  Target : {t['addr']}")
    try: r=subprocess.run([HYDRA,t['mask'],t['addr']],cwd=os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'), capture_output=True,text=True,timeout=120)
    except subprocess.TimeoutExpired: print("  ❌ TIMEOUT"); return False
    out=r.stdout+r.stderr
    if'VICTORY'in out:
        found=None
        for l in out.splitlines():
            if any(k in l for k in('VICTORY','WIF')): print(f"     {l.strip()}")
            m=re.search(r'(?:[KL][123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{51}|5[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{50})',l)
            if m: found=m.group(0)
        print("  ✅ PASS"+(f" (WIF vérifié)" if found==t['wif'] else"")); return True
    print(f"  ❌ FAIL (rc={r.returncode})\n  {out[-300:].strip()}"); return False

if __name__=='__main__':
    print(f"=== Hydra WIF test suite — {NUM_TESTS} tests × {NUM_UNKN} chars ===\nBinaire : {HYDRA}")
