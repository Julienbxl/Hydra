...
HYDRA=resolve_hydra()
NUM_TESTS=10; NUM_UNKN=5; WIF_LEN=52
ATYPES=['btc_legacy','btc_segwit']

def gen(i):
    k=random.randint(1,N-1); kb=k.to_bytes(32,'big')
    wif=wif_enc(kb); at=ATYPES[i%2]
    addr=btc_segwit(k) if at=='btc_segwit' else btc_legacy(k)
    s=random.randint(1,WIF_LEN-NUM_UNKN)
    return dict(i=i,kb=kb,wif=wif,addr=addr,at=at,mask=wif[:s]+'#'*NUM_UNKN+wif[s+NUM_UNKN:],s=s)

def run(t):
    print(f"\n{'='*62}\nTest #{t['i']+1:02d} | {t['at']}\n  WIF    : {t['wif']}\n  Masque : {t['mask']} (pos {t['s']}..{t['s']+NUM_UNKN-1})\n  Addr   : {t['addr']}")
    try: r=subprocess.run([HYDRA,t['mask'],t['addr']],capture_output=True,text=True,timeout=120)
    ...
    if'VICTORY'in out:
        ...
        for l in out.splitlines():
            ...
            m=re.search(r'[KL][123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{51}',l)
